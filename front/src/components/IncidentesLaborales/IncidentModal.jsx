...
import { X, FileText, Download, Calendar, User, AlertTriangle,CheckCircle,Flag,Eye,Save,Hash,Building, Upload, Trash2, Image } from 'lucide-react';
...
const IncidentModal = ({ isOpen, onClose, incident, onIncidentUpdate }) => {
  ...
  const fieldMap = {
    ...
    empresa: { alternatives: ['company', 'empresa', 'department', 'departamento'], default: generarEmpresa(incident?.id) },
  ...
  function generarEmpresa(incidentId) {
    const empresas = [
    ...
    if (!incidentId) return empresas[0];
    return empresas[(incidentId - 1) % empresas.length];
  }

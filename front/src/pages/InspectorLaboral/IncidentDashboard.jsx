...
const IncidentDashboardContent = () => {
  ...
  // Estado para los incidentes - inicialmente vacío, se cargan desde el backend
  const [incidentsData, setIncidentsData] = useState([]);
  const [isLoading, setIsLoading] = useState(true);

  // Cargar incidencias desde el backend al montar el componente
  useEffect(() => {
  ...
  const handleCreateIncident = async (newIncident) => {
    try {
      const response = await createIncidenciaRequest(newIncident);
      
      // Verificar que la respuesta sea exitosa
      if (response && response.success) {
        const nuevaIncidencia = response.data;
        setIncidentsData(prevData => [...prevData, nuevaIncidencia]);
        alert("✅ Incidencia creada correctamente");
      } else {
        // Si no es exitosa, mostrar error
        alert('❌ Error al crear la incidencia: Respuesta inesperada del servidor');
      }
    } catch (error) {
      console.error('Error creando incidencia:', error);
      // Mostrar mensaje de error más específico si está disponible
      const errorMessage = error.response?.data?.message || 'Error al crear la incidencia';
      alert(`❌ ${errorMessage}`);
    }
  };
  ...
  const handleIncidentUpdate = async (updatedIncident) => {
    try {
      const response = await updateIncidenciaRequest(updatedIncident.id, updatedIncident);
  ...
  const handleOcultarIncidente = async (incidentId) => {
    if (window.confirm("¿Estás seguro de que quieres ocultar este incidente?")) {
      try {
        await updateIncidenciaRequest(incidentId, { oculto: true });
        setIncidentsData(prevData =>
          prevData.map(incident =>
        ...
        console.error('Error ocultando incidencia:', error);
        alert('❌ Error al ocultar la incidencia');
      }
    }
  };

  // Función para eliminar un incidente
  const handleEliminarIncidente = async (incidentId) => {
    if (window.confirm("¿Estás seguro de que quieres ELIMINAR permanentemente este incidente? Esta acción no se puede deshacer.")) {
      try {
        await deleteIncidenciaRequest(incidentId);
        setIncidentsData(prevData =>
          prevData.filter(incident => incident.id !== incidentId)
        );
  ...
  const handleMostrarIncidente = async (incidentId) => {
    try {
      await updateIncidenciaRequest(incidentId, { oculto: false });
      setIncidentsData(prevData =>
        prevData.map(incident =>
          incident.id === incidentId
      ...
      console.error('Error mostrando incidencia:', error);
      alert('❌ Error al mostrar la incidencia');
    }
  };
  ...
  const getFilteredIncidents = () => {
    ...
    // Aplicar búsqueda con protección para propiedades undefined
    if (searchTerm) {
      filtered = filtered.filter(incident =>
        ...
        <div className={`flex-1 transition-all duration-300 ${collapsed ? 'ml-20' : 'ml-64'}`}>
          <div className="flex items-center justify-center min-h-screen">
            <div className="text-center">
              ...
              <p className="text-xl text-gray-600">Cargando incidencias...</p>
            </div>
          </div>
        </div>
      </div>
